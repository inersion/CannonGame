...
public class CannonView extends SurfaceView
   ...
   private Blocker blocker; //original blocker
   private Blocker blocker2; //additional blocker
   ...
   double interval; //interval period to update game elements
   double blockerPercent; //track the percent (in decimal) to increase blocker length

   // constants and variables for managing sounds
   public static final int TARGET_SOUND_ID = 0;
   public static final int CANNON_SOUND_ID = 1;
   public static final int BLOCKER_SOUND_ID = 2;
   public static final int WALL_SOUND_ID = 3;
   ...
   public CannonView(Context context, AttributeSet attrs) {
      ...
      //Added the sound for blocker or target hitting the top/bottom screen
      soundMap.put(WALL_SOUND_ID,
              soundPool.load(context, R.raw.two_pop, 1));
   ...
   public void playSound(int soundId) {
      //If it's determined that the cannon fire, blocker hit, or target hit sound is called
      if(soundId != 3) {
         //Turn off the volume of the "two_pop" sound when a target or blocker hits the top/bottom screen
         soundPool.play(soundMap.get(3), 0, 0, 0, 0, 1);

         //Play the cannon fire, blocker hit, or target hit sound at the normal rate
         soundPool.play(soundMap.get(soundId), 1, 1, 1, 0, 1);
      }
      //Play the "two_pop" sound when a target or blocker hits the top/bottom screen at the normal rate
   ...
   public void newGame() {
      ...
      // create a new additional Blocker that is has a different color/size/position
      blocker2 = new Blocker(this, Color.MAGENTA, MISS_PENALTY,
              (int) ((BLOCKER_X_PERCENT*0.75) * screenWidth),
              (int) ((-BLOCKER_LENGTH_PERCENT / 2) * screenHeight),
              (int) (BLOCKER_WIDTH_PERCENT * screenWidth),
              (int) ((BLOCKER_LENGTH_PERCENT / 2) * screenHeight),
              (float) ((BLOCKER_SPEED_PERCENT / 2) * screenHeight));
   ...
   private void updatePositions(double elapsedTimeMS) {
      interval = elapsedTimeMS / 1000.0; // convert to seconds
      ...
      blocker2.update(interval); //update the blocker2's position
   ...
   public void drawGameElements(Canvas canvas) {
      ...
      blocker2.draw(canvas); //draw the blocker2
   ...
   public void testForCollisions() {
      ...
      if (cannon.getCannonball() != null &&
         ...
         //variable track new blocker length percent
         double newBLP = BLOCKER_LENGTH_PERCENT;

         //increase blocker length by 5% (count initially set to 0.05)
         newBLP = newBLP * (1 + blockerPercent);

         //replace the current blocker with a new blocker with the new blocker length percent
         blocker = new Blocker(this, Color.BLACK, MISS_PENALTY,
                 (int) (BLOCKER_X_PERCENT * screenWidth),
                 (int) ((0.5 - BLOCKER_LENGTH_PERCENT / 2) * screenHeight),
                 (int) (BLOCKER_WIDTH_PERCENT * screenWidth),
                 (int) (newBLP * screenHeight),
                 (float) (BLOCKER_SPEED_PERCENT * screenHeight));

         //track and increase the count by 0.05 (5%) each time the cannonball hits the blocker
         blockerPercent = blockerPercent + 0.05;
      }

      // check if ball collides with blocker2
      if (cannon.getCannonball() != null &&
              (cannon.getCannonball().collidesWith(blocker2)) ) {
         blocker2.playSound(); // play blocker2 hit sound
   ...
   public void stopGame() {
      if (cannonThread != null) {
         ...
         //reset the blocker to the original parameters
         blocker = new Blocker(this, Color.BLACK, MISS_PENALTY,
                 (int) (BLOCKER_X_PERCENT * screenWidth),
                 (int) ((0.5 - BLOCKER_LENGTH_PERCENT / 2) * screenHeight),
                 (int) (BLOCKER_WIDTH_PERCENT * screenWidth),
   ...
   private class CannonThread extends Thread {
      ...
      public void run() {
         ...
         //initialize the percent (in decimal) to increase blocker length
         blockerPercent = 0.05;
